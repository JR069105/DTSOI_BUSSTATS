...
def get_buses():
    ...
    for post in posts:
        ...
        if ("hour" in timeAgo or "minute" in timeAgo) and ("hour" not in timeAgo or int(timeAgo.split(" ")[0]) <= 19) and ("bus" in (post.get_text()).lower() and "run" in (post.get_text()).lower()):
